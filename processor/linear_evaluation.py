...
class LE_Processor(Processor):
    ...
    def load_model(self):
        ...
        # if hasattr(self.model, 'encoder_motion'):
        #     for name, param in self.model.encoder_motion.named_parameters():
        #         if name not in ['fc.weight', 'fc.bias']:
        #             param.requires_grad = False
        #     self.num_grad_layers += 2            
    ...
    def load_optimizer(self):
        ...
        if self.arg.optimizer == 'SGD':
            ...
            self.optimizer = LARS(parameters,
                                    lr=self.arg.base_lr, 
                                    momentum=0.9, 
                                    weight_decay=self.arg.weight_decay)     
        else:
    ...
    def adjust_lr(self):
        if self.arg.step:
            ...
            lr = self.arg.base_lr *0.5 * (1. + math.cos(math.pi * (self.meta_info['epoch']) / (self.arg.num_epoch)))
            for param_group in self.optimizer.param_groups:
                param_group['lr'] = lr
            self.lr = lr            
