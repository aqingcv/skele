...
class MemoryBankModule(torch.nn.Module):
    ...
    @torch.no_grad()
    def _init_memory_bank(self, dim = 256):
        """Initialize the memory bank if it's empty
        Args:
            dim:
                The dimension of the which are stored in the bank.W
        """
...
class NNMemoryBankModule(MemoryBankModule):
    ...
    def forward(self,
        ...
        # nearest_neighbours, _ =  similarity_matrix.topk(k = 1, dim=1)   
        # similarity = torch.mm(output_normed, bank_normed.T)
        # nearest_neighbours, indices = similarity.topk(k = 5, dim=1) 
